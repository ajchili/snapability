...
class Stats extends Component {
  ...
  _fetchData = () => {
    axios
      .get(
        "https://us-central1-snapability-220017.cloudfunctions.net/fetchStatistics"
      )
      .then(res => {
        this.setState({
          ...
          switch (statistic.type) {
            case "twitter":
              if (!usagePerMinute[0].data[usage])
                usagePerMinute[0].data[usage] = 0;
              usagePerMinute[0].data[usage]++;
              break;
            case "tumblr":
              if (!usagePerMinute[1].data[usage])
                usagePerMinute[1].data[usage] = 0;
              usagePerMinute[1].data[usage]++;
              break;
            case "instagram":
              if (!usagePerMinute[2].data[usage])
                usagePerMinute[2].data[usage] = 0;
              ...
              >
                    ...
                    <div className="donut">
                      <Chart
                        options={{
                          ...
                          labels: ["Twitter", "Tumblr", "Instagram"]
                        }}
                        series={totalPlatformUsage}
                        type="donut"
                      />
                    </div>
                    <CardContent>
                ...
                <Grid item>
                  <Card>
                    <div className="line">
                      <Chart
                        options={usagePerMinuteOptions}
                        series={usagePerMinute}
                        type="line"
                      />
                    </div>
                    <CardContent>
